...
func TestBatchGetCanCollectAddress(t *testing.T) {
	client := transaction.NewEthClient(chainNet)
	if client == nil {
		panic("new wth client err")
	}
	defer client.Close()
	mgrAddresNum, tokenAddresNum, err := BatchGetCanCollectAddress(common.HexToAddress(contractAddress), 30, 10, client)
	t.Log(mgrAddresNum, tokenAddresNum, err)
}
