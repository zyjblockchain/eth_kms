...
func SendGasFeeForColAddr(startId, limit uint, client *ethclient.Client) error {
	...
	if err != nil {
		log.Errorf("SendGasFeeForColAddr error: %v", err)
	}
	log.Info("CollectionAddress total: ", total)
	// 为状态等于0的记录中的地址发eth
	collectionAddr, err := models.Get(models.CollectionAddrKey)
	if err != nil {
		log.Errorf("从数据库中查询collectionAddress error: %v", err)
		return err
	}
	// 获取gas 发送地址
	gasFromAddr, err := models.Get(models.GasFromAddrKey)
	if err != nil {
		log.Errorf("从数据库中查询gasFromAddr error: %v", err)
		return err
	}
	// 从kms中获取gas 发送地址私钥
	accKey, err := localcommon.GetAccountKeyByAddr(gasFromAddr)
	if err != nil {
		log.Errorf("从kms中获取gasFromAddr对应的私钥 error: %v", err)
		return err
	}
	...
	if err != nil {
		log.Errorf("获取gasFromAddr Nonce error: %v", err)
		return err
	}
	suggestGasPrice, err := client.SuggestGasPrice(context.Background())
	if err != nil {
		log.Errorf("获取suggest gasPrice 失败： %v", err)
	...
	for _, val := range collAddres {
		if val.State == 0 {
			...
			if err != nil {
				log.Errorf("发送eth 转gas交易error: %v", err)
				// 更新记录为发送失败
			...
			if err := txRecord.Update(models.Transaction{TxHash: tx.Hash().Hex(), State: 1}); err != nil {
				log.Errorf("更新transaction数据库error: %v", err)
				return err
			}
			// 开启一个协成来监听交易是否发送成功
			wg.Add(1)
				...
				for {
					if count > 12 {
						...
						if err := txRecord.Update(models.Transaction{State: 4}); err != nil {
							log.Errorf("交易监听超时，交易hash: %s", tx.Hash().Hex())
							return
						}
						return
					}
					...
					if (err != nil && err == ethereum.NotFound) || isPending {
						...
						if err := txRecord.Update(models.Transaction{State: 2, ErrMsg: err.Error()}); err != nil {
							log.Errorf("更新发送交易失败Transaction数据库error: %v", err)
							return
						}
					}
