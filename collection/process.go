...
func BatchGetCanCollectAddress(contractAddress common.Address, startId, limit uint, client *ethclient.Client) error {
	...
	for _, val := range keysMgrs {
		...
		if err != nil {
			// todo 目前对错误不处理， 直接跳过
			fmt.Errorf("查询地址token余额失败:%v", err)
			continue
		}
		// 判断balance是否为0
...
func saveByBatch(addInfos []*AddrTokenInfo) error {
	...
	if err != nil {
		fmt.Errorf("批量插入数据库(CollectionAddress) 失败: %v", err)
	}
	return err
}
...
func SendGasFeeForColAddr(startId, limit uint, client *ethclient.Client) error {
	...
	if err != nil {
		fmt.Errorf("SendGasFeeForColAddr error: %v", err)
	}
	fmt.Println("CollectionAddress total: ", total)
	// 为状态等于0的记录中的地址发eth
	collectionAddr, err := models.Get(models.CollectionAddrKey)
	if err != nil {
		fmt.Errorf("从数据库中查询collectionAddress error: %v", err)
		return err
	}
	// 获取gas 发送地址
	gasFromAddr, err := models.Get(models.GasFromAddrKey)
	if err != nil {
		fmt.Errorf("从数据库中查询gasFromAddr error: %v", err)
		return err
	}
	// 从kms中获取gas 发送地址私钥
	accKey, err := localcommon.GetAccountKeyByAddr(gasFromAddr)
	if err != nil {
		fmt.Errorf("从kms中获取gasFromAddr对应的私钥 error: %v", err)
		return err
	}
	...
	if err != nil {
		fmt.Errorf("获取gasFromAddr Nonce error: %v", err)
		return err
	}
	suggestGasPrice, err := client.SuggestGasPrice(context.Background())
	if err != nil {
		fmt.Errorf("获取suggest gasPrice 失败： %v", err)
	...
	for _, val := range collAddres {
		if val.State == 0 {
			...
			if err != nil {
				fmt.Errorf("发送eth 转gas交易error: %v", err)
				// 更新记录为发送失败
			...
			if err := txRecord.Update(models.Transaction{TxHash: tx.Hash().Hex(), State: 1}); err != nil {
				fmt.Errorf("更新transaction数据库error: %v", err)
				return err
			}
			// 开启一个协成来监听交易是否发送成功
			wg.Add(1)
				...
				for {
					if count > 12 {
						...
						if err := txRecord.Update(models.Transaction{State: 4}); err != nil {
							fmt.Errorf("交易监听超时，交易hash: %s", tx.Hash().Hex())
							return
						}
						return
					}
					...
					if (err != nil && err == ethereum.NotFound) || isPending {
						...
						if err := txRecord.Update(models.Transaction{State: 2, ErrMsg: err.Error()}); err != nil {
							fmt.Errorf("更新发送交易失败Transaction数据库error: %v", err)
							return
						}
					}
