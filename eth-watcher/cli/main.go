...
var rootCMD = &cobra.Command{
	Use:   "eth-watcher",
	Short: "eth-watcher makes getting updates from Ethereum easier",
}
...
var blockNumCMD = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		w := eth_watcher.NewHttpBasedEthWatcher(ctx, api)
...
var usdtTransferCMD = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		receiptLogWatcher := eth_watcher.NewReceiptLogWatcher(
			context.TODO(),
			api,
			-1,
			usdtContractAdx,
			topicsInterestedIn,
			handler,
			eth_watcher.ReceiptLogWatcherConfig{
				StepSizeForBigLag:               5,
...
var contractEventListenerCMD = &cobra.Command{
  ...
  /bin/eth-watcher contract-event-listener \
    --block-backoff 100
