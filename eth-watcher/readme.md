...
![](https://github.com/zyjblockchain/eth_kms/eth-watcher/workflows/Go/badge.svg)

eth-watcher is an event listener for the [Ethereum Blockchain](https://ethereum.org/) written in Golang. With eth-watcher you can monitor and track current or historic events that occur on the Ethereum Blockchain.

## Background

Many applications that interact with the Ethereum Blockchain need to know when specific actions occur on the chain, but cannot directly access the on-chain data. eth-watcher acts as an interface between application and chain: gathering specified data from the blockchain so that applications can more seamlessly interact with on-chain events.


## Features

1. Plug-in friendly. You can easily add a plugin to eth-watcher to listen to any type of on-chain event.
...
- [DefiWatch](https://defiwatch.io/) monitors the state of Ethereum addresses on several DeFi platforms including DDEX, Compound, DyDx, and Maker. It tracks things like loan ROI, current borrows, liquidations, etc. To track all of this on multiple platforms, DefiWatch has to continuously receive updates from their associated smart contracts. This is done using eth-watcher instead of spending time dealing with serialization/deserialization messages from the Ethereum Node, so they can focus on their core logic.   
- Profit & Loss calculations on [DDEX](https://ddex.io). DDEX provides their margin trading users with estimated Profit and Loss (P&L) calculations for their margin positions. To update the P&L as timely and accurately as possible, DDEX uses eth-watcher to listen to updates from the Ethereum Blockchain. These updates include: onchain price updates, trading actions from users, and more.
- DDEX also uses an "Eth-Transaction-Watcher" to monitor the on-chain status of trading transactions. DDEX needs to know the latest states of these transactions once they are included in newly mined blocks, so that the platform properly updates trading balances and histories. This is done using the `TxReceiptPlugin` of eth-watcher.


# Installation

Run `go get github.com/zyjblockchain/eth_kms/eth-watcher`

## Sample Commands

This project is primarily designed as a library to build upon. However, to help others easily understand how eth-watcher works and what it is capable of, we prepared some sample commands for you to try out.

**display basic help info**

```shell
docker run zyjblockchain/eth_kmsio/eth-watcher:master /bin/eth-watcher help

eth-watcher makes getting updates from Ethereum easier
...
Flags:
  -h, --help   help for eth-watcher

Use "eth-watcher [command] --help" for more information about a command.
```
...
```shell
docker run zyjblockchain/eth_kmsio/eth-watcher:master /bin/eth-watcher new-block-number
...
```shell
docker run zyjblockchain/eth_kmsio/eth-watcher:master /bin/eth-watcher usdt-transfer
...
```shell
docker run zyjblockchain/eth_kmsio/eth-watcher:master /bin/eth-watcher contract-event-listener \
    --block-backoff 100 \
...
...
```
Here the flag `--block-backoff` signals for eth-watcher to use historic tracking from 100 blocks ago.

# Usage

To effectively use eth-watcher, you will be interacting with two primary structs:
...
import (
	"context"
	"github.com/zyjblockchain/eth_kms/eth-watcher/blockchain"
	"github.com/sirupsen/logrus"
)
